...
if configure_static_ip; then
  ...
  if ip -4 addr show "${DHCP_INTERFACE}" | grep -q "inet " && [ "${DHCP_FORCE:-false}" != "true" ]; then
    ...
    if ! timeout "${DHCP_TIMEOUT:-15}" dhclient -1 -v "${DHCP_INTERFACE}"; then
      echo "DHCP request failed"
      if [ "${STATIC_IP_FALLBACK_PROMPT:-true}" = "true" ] && [ -t 0 ]; then
        echo "Offering static IP fallback"
        STATIC_IP_PROMPT=true configure_static_ip || echo "No static IP entered; continuing with existing address"
      else
        echo "Continuing with existing address"
      fi
    fi
  fi
fi
